#AMI
#jenkins-agent-linux-jnlp-01-v2.1
#ami-093d2024466a862c1


#jenkins-agent-windows-jnlp-01-v2.3
#ami-095fc69e304e5ae1f

  variable "montpoint"  {
    type = string
    default = "ssss"
  }
  
variable "health_check_path" {
  type = string
  default = "subnetpublicadobastisssssan"
}

variable "aws_cloudwatch_log_group_namedir"  {
  type = string
  default = "ssss"
}

 variable "windows_AMI" {
  type = string
  default = "ami-095fc69e304e5ae1f"
}

 variable "EFS_ID" {
  type = string
  default = ""
} 


variable "EFS_ID_ems" {
  type = string
  default = ""
}


 variable "container_definitions_name" {
  type = string
  default = ""
}


#############
 variable "final_container_definitions_name" {
  type = string
  default = ""
}


variable "final_AZregiaoDC" {
  type = string
  default = "wwrwt2.micro"
}

variable "final_regiaoDC" {
  type = string
  default = "wwwt2micro"
}
 

 variable "final_app_port" {
  type = string
  default = ""
}

variable "valor_aws_lb_target_group" {
  type = string
  default = "t112.micro"
}



variable "final_nometaskdefenitioncom_mont" {
  type = string
  default = "t332.micro"
}



variable "final_nometaskdefenitionsem_mont" {
  type = string
  default = "t4442.micro"
}



variable "final_aws_security_group__ecs_tasks" {
  type = string
  default = "t552.micro"
}


variable "final_nometaskdefenitionEFS_ID" {
  type = string
  default = "t662.micro"
}

variable "final_nometaskdefenition" {
  type = string
  default = "t772.micro"
}


variable "final_vpc_ldap_id_p" {
  type = string
  default = "t882.micro"
}


variable "final_fargate_cpu" {
  type = string
  default = "t9992.micro"
}




   variable "final_fargate_memory" {
  type = string
  default = "t2dddd.micro"
}

variable "final_app_image" {
  type = string
  default = "t2vvvv.micro"
}

variable "final_subnet_id" {
  type = string
  default = "t2vfvv.micro"
}

variable "final_SG_1" {
  type = string
  default = "t2ffggg  .micro"
}

######################
variable "nomedoALB"  {
  type = string
  default = "ssss"
}

variable "nomelisrener"  {
  type = string
  default = "ssss"
}

variable "prioridade"  {
  type = string
  default = "ssss"
}

variable "var_lb_main_id"  {
  type = string
  default = "ssss"
}


variable "dir"  {
  type = string
  default = "ssss"
}

variable "path"  {
  type = string
  default = "ssss"
}



variable "nomeALB"  {
  type = string
  default = "ddddddddfffff"
}

variable "port_alb_target_group" {
  type = string
  default = "wwrwt2.micro"
}


variable "container_port" {
  type = string
  default = "3890"
}

variable "final_albnome" {
  type = string
  default = "lixo"
}

 variable "final_alb_security_groups" {
  type = string
  default = ""
}

 variable "alb_security_groups" {
  type = string
  default = ""
} 

variable "AZregiaoDCB" {
  type = string
  default = "lllt2.micro"
}

 variable "psubnets_id_Alb" {
  type = string
  default = ""
} 

 

 variable "psubnetsb_id_Alb" {
  type = string
  default = ""
}

variable "final_psubnets_id_Ala" {
  type = string
  default = "subnetpublicadobastisssssan"
}


 variable "final_psubnetsb_id_Alb" {
  type = string
  default = ""
} 

 variable "vpc_ldap_id_p" {
  type = string
  default = ""
}



 variable "varvpcldapsubnet" {
  type = string
  default = ""
}

 variable "access_point_id_id" {
  type = string
  default = "hhhhh"
}



 variable "file_system_id" {
  type = string
  default = "umdenteer2o"
}


 variable "efs_name" {
  type = string
  default = "umdentr2o"
}
 variable "testeum" {
  type = string
  default = "umdentro"
}

 variable "testedois" {
  type = string
  default = "doisdentro"
}
#jenkins-master-linux-v2.1
#ami-093d2024466a862c1


####fargate inicio
 variable "cdir_vpc_ldap" {
  type = string
  default = "173.25.0.0/16"
}




 variable "fargate_cpu" {
  type = string
  default = ""
}


 variable "fargate_memory" {
  type = string
  default = ""
}

 variable "app_image" {
  type = string
  default = ""
}



variable "amiwindows" {
  type = string
  default = "ami-095fc69e304e5ae1f"
}


variable "amilinux" {
  type = string
  default = "ami-093d2024466a862c1mmm"
}



variable "subnet_a" {
  type = string
  default = "ami-093d2024466a862c1bbbbb"
}


##BAIXO

variable "subnet_cdir_subnetpubl_aza_ldap" {
  type = string
  default = "173.25.10.0/24"
}


variable "aws_subnet_private1_aza_id" {
  type = string
  default = "ec2-access-to-services-role"
}

variable "iam_instance_profile" {
  type = string
  default = "ec2-access-to-services-role"
}

variable "subnet_id" {
  type = string
  default = ""
}

 variable "app_port" {
  type = string
  default = ""
}



variable "aws_lb_application_arn" {
  type = string
  default = "t2.micro"
}

variable "aws_alb_target_group_arn" {
  type = string
  default = "t2.eeeemicro"
}

variable "final_ecs_servico_id" {
  type = string
  default = "t2.micro"
}

variable "final_vpc_id" {
  type = string
  default = ""
}

variable "sg_ldap" {
  type = string
  default = ""
}

variable "awslogs_group" {
  type = string
  default = "subnetpublicadobastisssssan"
}


variable "nometaskdefenition" {
  type = string
  default = ""
}

variable "nometaskdefenitionEFS_ID" {
  type = string
  default = ""
}

variable "nometaskdefenitionsem_mont" {
  type = string
  default = ""
}

variable "aws_security_group__ecs_tasks" {
  type = string
  default = ""
}


variable "nometaskdefenitioncom_mont" {
  type = string
  default = ""
}




variable "ALBnome" {
  description = "The name for the resource"
  default = "example"
}


variable "ALB_SG_ID" {
  type = string
  default = ""
}


variable "VPC_LDAP_ID" {
  type = string
  default = ""
}

variable "sem_mont" {
  type = string
  default = "0"
}

variable "vpc_ldap_id" {
  type = string
  default = ""
}



variable "bucketnaame" {
  type = string
  default = "bucketnaame"
}

variable "ec2id" {
  type = string
  default = "wwrwt2.micro"
}



variable "AZregiaoDC" {
  type = string
  default = "wwrwt2.micro"
}

variable "regiaoDC" {
  type = string
  default = "wwwt2micro"
}

variable "key_name" {
  type = string
  default = "t2.micro"
}
###CIMA

variable "sem_mount" {
  description = "AWS region"
  type = string
  default = "0"
}

variable "monitoring" {
  type = string
  default = "t2.micro"
}

variable "instance_type" {
  type = string
  default = "t2.micro"
}


variable "availability_zone" {
  type = string
  default = "eu-west-3a"
}


variable "amilinux_bastian" {
  type = string
  default = "ami-093d2024466a862c1"
}

variable "ami_id" {
  type = string
  default = "ami-0d3c032f5934e1b41"
}

variable "amilinux_master" {
  type = string
  default = "ami-093d2024466a862c1xxxx"


}



variable "cidr_blocks" {
  type = string
  default = ""
}

variable "aws_vpc_vpc_cross_id" {
  type = string
  default = ""
}



variable "subnet_nome_subnetpublic_bzb" {
  type = string
  default = "subnet_nome_subnetpublic_bzb"
}



variable "novaamilinux" {
  type = string
  default = "ami-055575a0c2704effa"
}

variable "nome_SG_ec2" {
  type = string
  default = "L000~rrr0LL"
}

variable "nome_SG_ec2_batian" {
  type = string
  default = "scib-cross-sg8"
}

variable "nome_SG_jpteste" {
  type = string
  default = "scib-cross-sg8"
}

variable "SG_1" {
  type = string
  default = ""
}

variable "bastian_linux" {
  type = string
  default = "scib-cross-ec2-bastian"
}



variable "jenkins_server_linux" {
  type = string
  default = "scib-cross-ec2-jenkins_server"
}

variable "jenkins_slave_linux" {
  type = string
  default = "scib-cross-ec2-jenkins_slave"
}



variable "jenkins_slave_windows" {
  type = string
  default = "scib-cross-ec2-jslave_windows"
}



variable "nome_vpc_ldap" {
  type = string
  default = "scib-cross-vpc-ldap"
}

variable "routetable_name_ldap" {
  type = string
  default = "scib-cross-routetable-ldap"
}

variable "subnet_cdir_subnetpubl_ldap_aza" {
  type = string
  default = "173.25.10.0/24"
}


variable "fazer_subnet_privada" {
  type = string
  default = "1"
}




###Fargate fim

variable "nome_vpc" {
  type = string
  default = "scib-cross-vpc"
}


variable "subnet_nome_subnetpublica_aza" {
  type = string
  default = "subnetpublicadobaseeeetian"
}

variable "subnet_cdir_subpublica_aza" {
  type = string
  default = "10.0.11.0/24"
}


#subnetAZ publica
variable "subnet_cdir_subpublica_nat_aza" {
  type = string
  default = "10.0.31.0/24"
}

variable "subnet_cdir_subpublica_nat_azb" {
  type = string
  default = "10.0.61.0/24"
}

variable "subnet_cdir_subpublica_nat_azc" {
  type = string
  default = "10.0.91.0/24"
}


###fim subnet publica

variable "nome_ldap_vpc" {
  type = string
  default = "scib-cross-vpc-lpad"
}

variable "cdir_vpc" {
  type = string
  default = "10.0.0.0/16"
}


variable "subnet_nome_subnetpublic_aza" {
  type = string
  default = "subnetpublicaAVAZA"
}

variable "subnet_nome_subnetpunlic_azb" {
  type = string
  default = "subnetpublicaAVAZB"
}

variable "subnet_nome_subnetpublic_azc" {
  type = string
  default = "subnetpublicaAVAZC"
}

variable "cidir_da_vpc_do_ldap" {
  type = string
  default = "172.24.0.0/16"
}

variable "subnet_nome_subnetpriv_aza" {
  type = string
  default = "jpsubnetprivazav1"
}

variable "subnet_cdir_subnetpriv_aza" {
  type = string
  default = "10.0.11.0/24"
}







variable "subnet_nome_subnetpriv_azb" {
  type = string
  default = "jpsubnetprivazb"
}

variable "subnet_cdir_subnetpriv_azb" {
  type = string
  default = "10.0.2.0/24"
}

variable "subnet_nome_subnetpriv_azc" {
  type = string
  default = "jpsubnetprivazc"
}

variable "subnet_cdir_subnetpriv_azc" {
  type = string
  default = "10.0.3.0/24"
}

variable "billing" {
  type = string
  default = "cross"
}


variable "tag1" {
  type = string
  default = "tag1"
}


variable "tag2" {
  type = string
  default = "tag2"
}


variable "filepolicy" {
  type = string
  default = "L9LL"
}



variable "igw_name" {
  type = string
  default = "scib-cross-IGW"
}


variable "routetable_name" {
  type = string
  default = "scib-cross-routetable"
}



variable "ec2nome" {
  type = string
  default = "ec2nome"
}

variable "SG_RDS" {
  type = string
  default = "ec2nome"
}



variable "efsldap" {
  type = string
  default = "efsldap"
}


variable "subnet_id_puba" {
  type = string
  default = "ec2nome"
}


variable "ec2_name" {
  type = string
  default = "ec2nome"
}

variable "CIA_value" {
  type = string
  default = "L6654LL"
}

variable "port_alb_listener" {
  type = string
  default = ""
}


variable "amid" {
  type = string
  default = "ami-05d0610075506aac2"
}


variable "key_par_name" {
  type = string
  default = "finatakp"
}


variable "endpointrds" {
  type = string
  default = "ec2-key"
}

variable "iamrole_name" {
  type = string
  default = "instancerole1"
}

variable "publicdestCIDR" {
  type = string
  default = "0.0.0.0/0"
}



variable "HC_port" {
  type = string
  default = "0.0.0.0/0"
}


variable "root_directory_efs" {
  type = string
  default = "172.24.0.0/16"
}


variable "ldapvpcCIDR" {
  type = string
  default = "172.24.0.0/16"
}

variable "access_point" {
  type = string
  default = "efss333igom"
}

variable "containerPath" {
  type = string
  default = "172.24.0.0/16"
}


#####Fargatevariaveis

variable "ldap_cluster_name" {
  type = string
  default = "scib-cross-services-fargate"
}




variable "bastian_ip_privado" {
  type = string
  default = ""
}


variable "user_data" {
  type = string
  default = "scib-cross-services-fargate"
}

/* variable "nome_SG_bastian" {
  type = string
  default = "scib-cross-services-fargate"
} */




variable "cdir_ldap_vpc" {
  type = string
  default = ""
}


variable "subnet_cdir_subnetpriv_aza_ldap" {
  type = string
  default = ""
}


variable "subnet_nome_subnetpriv_aza_ldap" {
  type = string
  default = ""
}





variable "sg_ec2" {
  type = string
  default = "sssddd"
}

variable "sg_efs2" {
  type = string
  default = ""
}

variable "nomeec2jenkins_slave_linux" {
  type = string
  default = "wwwt2.micro"
}

variable "ec2_name_jenkins_server_linux" {
  type = string
  default = "wwwt2.micro"
}

variable "nome_SG_bastian" {
  type = string
  default = "wwwt2.micro"
}



##RDS



variable "subnet1" {
  type = string
  default = "wwwt2.micro"
}

variable "subnet2" {
  type = string
  default = "wwwt2.micro"
}


variable "snapshot" {
  type = string
  default = "wwwt2.micro"
}

variable "final_snapshot" {
  type = string
  default = "wwwt2.micro"
}
